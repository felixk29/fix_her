...
class FourRoomsEnv(MiniGridEnv):
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None):
        ...
        #     print("loading state")
        #     load_state = options["load_state"]
        #     self._load_state_full(load_state)
        #     return self.gen_obs(), {}
    
        #generate new state but keep agent position from "loaded" state
        ...
        if options and "load_state" in options:
            ...
            load_state = options["load_state"]
            self._load_state_position(load_state)
            ob=self.gen_obs()
            self.step_count = 0
    ...
    #load whole state 
    def _load_state_full(self, state):

        ret=obs_to_state(state)
        self.agent_pos= (ret[0], ret[1])
        self.agent_dir= ret[2]
        self.goal_pos= (ret[3], ret[4])
        self.doors_pos= (ret[5], ret[6], ret[7], ret[8])
        self._gen_grid(self.width, self.height, lock=True)

    #load only position of state
    def _load_state_position(self, state):
    ...
    #load only agent position and direction, generate new goal and door positions
    # this is poisened for some reason, kept as a warning, stops doing anything after  1.5
    # def _load_state(self, state, seed):
    #     print("loading state - 1")
    #     super().reset(seed=seed)
    #     print("loading state - 1.5")
    #     obs=self.gen_obs()
    #     print(obs)
    #     og=obs_to_state(obs)
    #     self.goal_pos= (og[3], og[4])
    #     self.doors_pos= (og[5], og[6], og[7], og[8])
    ...
    def _gen_grid(self, width, height, lock=False):
        # Create the grid
        ...
        if lock:
            doors_pos=self.doors_pos
            agent_dir=self.agent_dir
        elif self._doors_pos_list is not None:
        ...
        elif self._agent_pos_list is not None:
        ...
        elif self._goal_pos_list is not None:
            goal = Goal()
        ...
        if (self._agent_pos_list is not None) and not lock:
